...
def test_init():
    ...
    assert tree.max_depth == 3 and tree.min_size == 2 and tree.n_features == 1
...
test_split_data = [
    (
        np.array([[1, 4, 3, 0], [2, 3, 3, 0], [3, 2, 3, 0], [4, 1, 3, 1]]),
        0,
        3,
        [
            np.array([[1, 4, 3, 0], [2, 3, 3, 0]]),
            np.array([[3, 2, 3, 0], [4, 1, 3, 1]]),
        ],
    ),
    (
        np.array([[1, 4, 3, 0], [2, 3, 3, 0], [3, 2, 3, 0], [4, 1, 3, 1]]),
        ...
        [
            np.array([[4, 1, 3, 1]]),
            np.array([[1, 4, 3, 0], [2, 3, 3, 0], [3, 2, 3, 0]]),
        ],
    ),
]
...
def test_split_dataset(data, column, value, expected):
    ...
    result = tree._test_split(dataset=data, column=column, value=value)

    assert np.array_equal(result[0], expected[0]) & np.array_equal(
        result[0], expected[0]
    )


predict_tests = [
    (np.array([1, 2]), {"column": 0, "value": 2, "left": 1}, 1),
    (np.array([1, 2]), {"column": 0, "value": 1, "right": 0}, 0),
    (np.array([1, 2]), {"column": 1, "value": 3, "left": 1}, 1),
    (
        np.array([1, 2]),
        {"column": 0, "value": 1, "right": {"column": 1, "value": 5, "left": 0}},
        0,
    ),
    (
        np.array([1, 2]),
        {"column": 0, "value": 2, "left": {"column": 1, "value": 1, "right": 1}},
        1,
    ),
]
