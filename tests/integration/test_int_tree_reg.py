...
def test__get_split():
    ...
    right_grp = np.array_equal(
        result["groups"][1], np.array([[0.5, 0.5], [0.7, 0.7], [0.9, 0.9]])
    )
...
public_predict_test = [
    (
        np.array([[0.0, 0.1], [0.0, 0.5], [0.0, 0.7], [0.0, 0.9]]),  # X
        np.array([0.1, 0.5, 0.7, 0.9]),  # y
        np.array([0.1, 0.6, 0.6, 0.9]),
    ),  # y_test
    (
        pd.DataFrame({"c1": [0.0, 0.0, 0.0, 0.0], "c2": [0.1, 0.5, 0.7, 0.9]}),  # X
...
def test_score():
    X = pd.DataFrame({"c1": [0.0, 0.0, 0.0, 0.0], "c2": [0.1, 0.5, 0.7, 0.9]})
